...
gl = operation_context = runtime = storage = None


def _load_opengl_modules() -> None:
    """Load the legacy OpenGL-only probe after backend selection."""
    global gl, operation_context, runtime, storage
...
def main() -> int:
    ...
    if selected.name != "opengl":
        print("opengl_address_diagnostic requires the OpenGL backend.")
        print(f"Selected backend: {selected.name.upper()}")
        return 0
    _load_opengl_modules()
    gm45.set_trace(False)
    print("OpenGL packed-address diagnostic (no convolution)")
