#!/usr/bin/env python3
"""
Ultralytics YOLO operator discovery for the experimental MatrixMan backend.

This script does not silently run unsupported MatrixMan tensor arithmetic on CPU.
...
The report is the authority for which MatrixMan backend operators to implement next.
"""
...
def print_progress_summary(recorder: AtenRecorder, failure: str | None, progress: dict[str, Any]) -> None:
    ...
    print("\nMatrixMan YOLO compatibility:")
    print("  model: yolov8n.yaml")
    print(
        "  model layers reached: "
        f"{progress['entered']} entered / {progress['completed']} completed / {progress['total_layers']} total"
    )
    print("  supported ATen calls with current MatrixMan backend:", supported_calls)
    ...
    print("  current MatrixMan progress: NCHW input upload works; forward stops at first unsupported MatrixMan op")


def main() -> int:
    parser = argparse.ArgumentParser(description="Discover Ultralytics YOLO ATen ops for MatrixMan backend work")
    ...
    print("Ultralytics YOLO MatrixMan operation discovery")
    print("  model spec:", args.model)
    print("  dummy input:", [1, 3, args.imgsz, args.imgsz], "float32")
    print("  discovery execution: PyTorch FakeTensor/meta; no CPU tensor arithmetic fallback is claimed as MatrixMan")

    real_blocker = run_real_matrixman_first_blocker(args.model, args.imgsz)
