#!/usr/bin/env python3
"""Standalone OpenGL convolution arithmetic probes.
...
gl = convolution = operation_context = resources = runtime = storage = None
GL_SCISSOR_TEST = 0x0C11


def _load_opengl_modules() -> None:
    """Load OpenGL arithmetic-probe dependencies only for OpenGL selection."""
    global gl, convolution, operation_context, resources, runtime, storage
    from drivers.matrixman import gpumatrix as gl_module
    from drivers.matrixman.backends.opengl import convolution as convolution_module
    from drivers.matrixman.backends.opengl import operation_context as context_module
    from drivers.matrixman.backends.opengl import resources as resources_module
    from drivers.matrixman.backends.opengl import runtime as runtime_module
    from drivers.matrixman.backends.opengl import storage as storage_module

    gl = gl_module
    convolution = convolution_module
    operation_context = context_module
...
def main():
    ...
    if selected.name != "opengl":
        print("opengl_conv_arithmetic_diagnostic requires the OpenGL backend.")
        print(f"Selected backend: {selected.name.upper()}")
        return 0
    _load_opengl_modules()
    ...
    if "--dispatch-only" in sys.argv:
        ...
        gm45.shutdown()
        return
    print("OpenGL convolution arithmetic diagnostic; MatrixMan convolution is not called")
