...
def _load_opengl_modules() -> None:
    """Load physical-target OpenGL dependencies only after selection."""
...
def main():
    ...
    if selected.name != "opengl":
        print("opengl_conv_target_diagnostic requires the OpenGL backend.")
    ...
    if "--production-tiles" in sys.argv:
        ...
        gm45.shutdown()
        return
    print("OpenGL physical convolution target diagnostic; no backend/demo changes")
