#!/usr/bin/env python3
"""Step 10B selected-backend Conv2D versus optional OpenGL telemetry diagnostic."""
...
def _load_opengl_profiling() -> None:
    global profiling
    from drivers.matrixman.backends.opengl import profiling as profiling_module
...
def _run_tiny_check(gpu_input, weight, *, spatial_reuse: bool) -> bool:
    ...
    label = "spatial_reuse" if spatial_reuse else "selected_backend"
    print(f"    {label}_flat={actual.reshape(-1).tolist()}")
...
def main() -> int:
    ...
    is_opengl = selected.name == "opengl"
    if is_opengl:
        _load_opengl_profiling()
    ...
    try:
        ...
        if not is_opengl:
            print("  OpenGL spatial-reuse profiling: skipped (telemetry is OpenGL-specific)")
        print("  input: [1,64,80,80]")
        print("  weight: [64,64,3,3]")
        print("  output: [1,64,80,80]")
        print("  stride=1 padding=1 dilation=1 groups=1")
        print(f"  atlas: {'320x320' if is_opengl else 'not applicable (CUDA linear storage)'}")
        ...
        tiny_passed = _run_tiny_check(
            matrixman.to_device(tiny_source), tiny_weight, spatial_reuse=is_opengl
        )
